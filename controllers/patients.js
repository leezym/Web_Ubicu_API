...
export async function createPatient(req, resp) {
...
export async function createPatientWithDefaults(req, res) {
    const requiredFields = ['nombre', 'cedula', 'telefono', 'email', 'edad', 'sexo', 'peso', 'altura', 'direccion', 'ciudad', 'password', 'id_user'];
    if (!validateRequiredFields(req.body, requiredFields)) {
...
export async function updatePatient(req, resp) {
...
export async function getPatientbyId(req, resp) {
...
export async function getPatientbyCc(req, resp) {
...
export async function getPatientbyUser(req, resp) {
    if (!validateRequiredFields(req.body, ['id_user'])) {
        return resp.status(400).send({ msg: "Faltan datos." });
    }
    const { id_user } = req.body;
    const objectId = new mongo.Types.ObjectId(id_user);
    try {
...
export async function authenticatePatient(req, res) {
    if (!validateRequiredFields(req.body, ['cedula', 'password'])) {
        return res.status(400).send({ msg: "Faltan datos." });
    }
    const { cedula, password } = req.body;
